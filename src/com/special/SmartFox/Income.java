...
public class Income extends Activity implements OnClickListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		db.execSQL("CREATE TABLE IF NOT EXISTS user_incm(name VARCHAR,balance LONGINT,currency VARCHAR,catagory VARCHAR,other VARCHAR);");
	}
	...
	public void onClick(View view) {
		if (view == btnAdd) {
			...
			db.execSQL("INSERT INTO user_incm VALUES('" + name.getText() + "','"
		...
		if (view == btnDelete) {
			...
			Cursor c = db.rawQuery(
					"SELECT * FROM user_incm WHERE name='" + name.getText()
							+ "'", null);
			if (c.moveToFirst()) {
				db.execSQL("DELETE FROM user_incm WHERE name='" + name.getText()
						+ "'");
		...
		if (view == btnModify) {
			...
			Cursor c = db.rawQuery(
					"SELECT * FROM user_incm WHERE Name='" + name.getText()
							+ "'", null);
			if (c.moveToFirst()) {
				db.execSQL("UPDATE user_incm SET balance='" + balance.getText()
		...
		if (view == btnViewAll) {
			Cursor c = db.rawQuery("SELECT * FROM user_incm", null);
			if (c.getCount() == 0) {
			...
			showMessage("Income Info", buffer.toString());
		}
