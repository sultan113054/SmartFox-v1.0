...
public class Expense extends Activity implements OnClickListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		db.execSQL("CREATE TABLE IF NOT EXISTS user_expns(name VARCHAR,balance LONGINT,currency VARCHAR,catagory VARCHAR,other VARCHAR);");
	}
	...
	public void onClick(View view) {
		if (view == btnAdd) {
			...
			db.execSQL("INSERT INTO user_expns VALUES('" + name.getText() + "','"
		...
		if (view == btnDelete) {
			...
			Cursor c = db.rawQuery(
					"SELECT * FROM user_expns WHERE name='" + name.getText()
							+ "'", null);
			if (c.moveToFirst()) {
				db.execSQL("DELETE FROM user_expns WHERE name='" + name.getText()
						+ "'");
		...
		if (view == btnModify) {
			...
			Cursor c = db.rawQuery(
					"SELECT * FROM user_expns WHERE Name='" + name.getText()
							+ "'", null);
			if (c.moveToFirst()) {
				db.execSQL("UPDATE user_expns SET balance='" + balance.getText()
		...
		if (view == btnViewAll) {
			Cursor c = db.rawQuery("SELECT * FROM user_expns", null);
			if (c.getCount() == 0) {
			...
			showMessage("Expense Info", buffer.toString());
		}
