...
public class Planning extends Activity implements OnClickListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		db.execSQL("CREATE TABLE IF NOT EXISTS user_plan(name VARCHAR,balance LONGINT,currency VARCHAR,catagory VARCHAR,other VARCHAR);");
	}
	...
	public void onClick(View view) {
		if (view == btnAdd) {
			...
			db.execSQL("INSERT INTO user_plan VALUES('" + name.getText() + "','"
		...
		if (view == btnDelete) {
			...
			Cursor c = db.rawQuery(
					"SELECT * FROM user_plan WHERE name='" + name.getText()
							+ "'", null);
			if (c.moveToFirst()) {
				db.execSQL("DELETE FROM user_plan WHERE name='" + name.getText()
						+ "'");
		...
		if (view == btnModify) {
			...
			Cursor c = db.rawQuery(
					"SELECT * FROM user_plan WHERE Name='" + name.getText()
							+ "'", null);
			if (c.moveToFirst()) {
				db.execSQL("UPDATE user_plan SET balance='" + balance.getText()
		...
		if (view == btnViewAll) {
			...
			while (c.moveToNext()) {
				...
				buffer.append("Details: " + c.getString(3) + "\n");
				buffer.append("Type: " + c.getString(4) + "\n\n");
			}
			showMessage("Planning Info", buffer.toString());
		}
