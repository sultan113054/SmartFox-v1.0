...
public class Transfer extends Activity implements OnClickListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		db.execSQL("CREATE TABLE IF NOT EXISTS user_trnfr(name VARCHAR,balance LONGINT,currency VARCHAR,catagory VARCHAR,other VARCHAR);");
	}
	...
	public void onClick(View view) {
		if (view == btnAdd) {
			...
			db.execSQL("INSERT INTO user_trnfr VALUES('" + name.getText() + "','"
		...
		if (view == btnDelete) {
			...
			Cursor c = db.rawQuery(
					"SELECT * FROM user_trnfr WHERE name='" + name.getText()
							+ "'", null);
			if (c.moveToFirst()) {
				db.execSQL("DELETE FROM user_trnfr WHERE name='" + name.getText()
						+ "'");
		...
		if (view == btnModify) {
			...
			Cursor c = db.rawQuery(
					"SELECT * FROM user_trnfr WHERE Name='" + name.getText()
							+ "'", null);
			if (c.moveToFirst()) {
				db.execSQL("UPDATE user_trnfr SET balance='" + balance.getText()
		...
		if (view == btnViewAll) {
			Cursor c = db.rawQuery("SELECT * FROM user_trnfr", null);
			if (c.getCount() == 0) {
			...
			while (c.moveToNext()) {
				...
				buffer.append("From ACC: " + c.getString(2) + "\n");
				buffer.append("To ACC: " + c.getString(3) + "\n");
				buffer.append("Currency Rate: " + c.getString(4) + "\n\n");
			}
			showMessage("Transfer Info", buffer.toString());
		}
