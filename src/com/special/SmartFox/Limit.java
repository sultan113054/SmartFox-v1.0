...
public class Limit extends Activity implements OnClickListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		db.execSQL("CREATE TABLE IF NOT EXISTS user_acc(name VARCHAR,balance LONGINT,currency VARCHAR,catagory VARCHAR,other VARCHAR);");
	}
	...
	public void onClick(View view) {
		if (view == btnAdd) {
			...
			db.execSQL("INSERT INTO user_acc VALUES('" + name.getText() + "','"
		...
		if (view == btnDelete) {
			...
			Cursor c = db.rawQuery(
					"SELECT * FROM user_acc WHERE name='" + name.getText()
							+ "'", null);
			if (c.moveToFirst()) {
				db.execSQL("DELETE FROM user_acc WHERE name='" + name.getText()
						+ "'");
		...
		if (view == btnModify) {
			...
			Cursor c = db.rawQuery(
					"SELECT * FROM user_acc WHERE Name='" + name.getText()
							+ "'", null);
			if (c.moveToFirst()) {
				db.execSQL("UPDATE user_acc SET balance='" + balance.getText()
		...
		if (view == btnViewAll) {
			Cursor c = db.rawQuery("SELECT * FROM user_acc", null);
			if (c.getCount() == 0) {
			...
			while (c.moveToNext()) {
				...
				buffer.append("Balance: " + c.getString(1) + "\n");
				buffer.append("Currency: " + c.getString(2) + "\n");
				buffer.append("Category: " + c.getString(3) + "\n");
				buffer.append("Other Info: " + c.getString(4) + "\n\n");
			}
			showMessage("Account Info", buffer.toString());
		}
